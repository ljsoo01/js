  ...
  const timeline = posts.map(post => (
    <li>
      <Link to={`/p/${post.id}`} className="block h-40 relative">
        ...
        <div className="absolute inset-0 bg-black/[0.2] flex flex-col justify-ccenter opacity-0 hover:opacity-100">
          <div className="flex justify-center">
            <FaHeart className="fill-white" size="20" />
            <span className="ml-2 text-white">{post.likeCount}</span>
          </div>
          <div className="flex justify-center">
    ...
    <div className="mt-8">
      ...
      {!isMaster && (
        <div className="mt-8 grid grid-cols-2 gap-2 px-4">
          {profile.isFollowing ? (
            <button 
              className="bg-gray-200 text-sm px-4 py-2 font-semibold p-2 roudned-lg"
